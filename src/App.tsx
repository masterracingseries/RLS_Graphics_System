    ...
    <div className="absolute inset-0 overflow-hidden" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#05070a' }}>
      ...
      <div className="absolute inset-0 z-0">
        <img src={getExportUrl(data.background)} alt="" className="w-full h-full object-cover"
          referrerPolicy="no-referrer" crossOrigin={getCrossOrigin(getExportUrl(data.background))} onError={handleBgError} />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(4,6,10,0.55) 0%, rgba(4,6,10,0.25) 30%, rgba(5,7,10,0.55) 58%, #05070a 82%)' }} />
      </div>

      {/* Etiqueta RACE RESULT (arriba izquierda) */}
      <div className="absolute top-[18px] left-4 z-30 transform -skew-x-12 px-4 py-1" style={{ backgroundColor: color }}>
        <span className="block transform skew-x-12 text-[12px] font-black uppercase tracking-[0.25em] italic whitespace-nowrap" style={{ color: color === '#ffffff' ? '#000' : '#fff' }}>Race Result</span>
      </div>
      ...
      <div className="absolute top-[42px] left-4 z-30 leading-none">
        ...
        <div className="text-[12px] font-bold uppercase tracking-[0.35em] mt-0.5" style={{ color }}>Position</div>
      </div>

      {/* Logo RLS + marca (arriba derecha) */}
      <div className="absolute top-[18px] right-4 z-30 flex flex-col items-center">
        <img src={getExportUrl(RLS_LOGO_URL)} alt="RLS" className="h-12 object-contain drop-shadow-[0_2px_10px_rgba(0,0,0,1)]"
          referrerPolicy="no-referrer" crossOrigin={getCrossOrigin(getExportUrl(RLS_LOGO_URL))} />
        <span className="text-[7px] font-bold uppercase tracking-[0.2em] text-white/70 mt-0.5 whitespace-nowrap">Racing Latam Esport</span>
      </div>

      {/* Foto piloto */}
      <div className="absolute inset-x-0 top-[64px] z-[5] flex justify-center" style={{ height: '40%' }}>
        {data.image ? (
          <div className="relative h-full w-[78%] flex items-end justify-center">
            <img src={getExportUrl(data.image)} alt="Pilot" className="h-full w-full object-contain object-bottom"
              referrerPolicy="no-referrer" crossOrigin={getCrossOrigin(getExportUrl(data.image))} />
          </div>
        ) : (
      ...
      <div className="absolute left-0 w-full z-20 px-3" style={{ top: '53%' }}>
        <div className="relative flex items-center">
          <div className="w-4 h-7 flex-shrink-0" style={{ backgroundColor: color, transform: 'skewX(-20deg)', marginRight: '-6px' }} />
          <div className="flex-1 py-1.5 px-3 text-center" style={{ backgroundColor: 'rgba(6,10,14,0.86)', borderTop: `1px solid ${color}55`, borderBottom: `1px solid ${color}55` }}>
            <div className={`text-white font-black italic uppercase tracking-tight leading-none ${longName ? 'text-lg' : 'text-xl'}`}>{data.nickname || 'PILOTO'}</div>
            <div className="text-[9px] text-white/50 uppercase tracking-[0.25em] mt-0.5">{data.realName}</div>
          </div>
          <div className="w-4 h-7 flex-shrink-0" style={{ backgroundColor: color, transform: 'skewX(-20deg)', marginLeft: '-6px' }} />
        </div>
        <div className="text-center text-[9px] font-bold uppercase tracking-[0.3em] text-white/70 mt-1">{data.league}</div>
      </div>

      {/* Resplandor detrás del auto, para que se despegue del fondo */}
      <div className="absolute left-1/2 -translate-x-1/2 w-[80%] h-[70px] z-[10] pointer-events-none rounded-full"
        style={{ top: '67%', background: `radial-gradient(ellipse at center, ${color}3d 0%, transparent 72%)`, filter: 'blur(6px)' }} />

      {/* Auto protagonista, nítido y grande */}
      <div className="absolute left-1/2 -translate-x-1/2 w-[97%] h-[118px] z-[15] pointer-events-none" style={{ top: '65.5%' }}>
        <img src={getExportUrl(team.car)} alt="" className="w-full h-full object-contain drop-shadow-[0_12px_22px_rgba(0,0,0,0.75)]"
          style={{ transform: `scale(${team.carScale ?? 1})`, filter: 'contrast(1.12) saturate(1.12) brightness(1.06)' }}
          referrerPolicy="no-referrer" crossOrigin={getCrossOrigin(getExportUrl(team.car))} />
      </div>

      {/* Barra de estadísticas */}
      <div className="absolute left-3 right-3 z-20 flex items-stretch justify-between rounded-xl px-2 py-2" style={{ bottom: '32px', backgroundColor: 'rgba(6,10,14,0.82)', border: `1px solid ${color}44` }}>
        {stats.map(([label, value], i) => (
          <div key={label} className="flex-1 flex flex-col items-center justify-center px-1" style={{ borderLeft: i > 0 ? `1px solid ${color}33` : 'none' }}>
            ...
            <span className="text-white font-black uppercase leading-none mt-1" style={{ fontSize: value.length > 3 ? '13px' : '17px' }}>{value}</span>
          </div>
        ))}
      </div>

      {/* Footer */}
      <div className="absolute bottom-[8px] left-0 w-full flex items-center justify-center gap-2 z-20">
