...
function EsportsTemplate({ data, team, circuit }: TemplateProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#05070a' }}>
      ...
      {/* Foto piloto */}
      <div className="absolute inset-x-0 top-[64px] z-[5] flex justify-center" style={{ height: '38%' }}>
        {data.image ? (
      ...
      <div className="absolute left-0 w-full z-20 px-3" style={{ top: '45%' }}>
        <div className="relative flex items-center">
          <div className="w-4 h-7 flex-shrink-0" style={{ backgroundColor: color, transform: 'skewX(-20deg)', marginRight: '-6px' }} />
          <div className="flex-1 py-1.5 px-3 text-center" style={{ backgroundColor: 'rgba(6,10,14,0.9)', borderTop: `1px solid ${color}88`, borderBottom: `2px solid ${color}` }}>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 w-[100%] h-[152px] z-[15] pointer-events-none" style={{ top: '57.5%' }}>
        <img src={getExportUrl(team.car)} alt="" className="w-full h-full object-contain"
          style={{ transform: `scale(${team.carScale ?? 1})`, filter: 'contrast(1.2) saturate(1.3) brightness(1.1) drop-shadow(0 16px 18px rgba(0,0,0,0.8))' }}
          referrerPolicy="no-referrer" crossOrigin={getCrossOrigin(getExportUrl(team.car))} />
      </div>
