...
function EsportsTemplate({ data, team, circuit }: TemplateProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ fontFamily: "'Inter', sans-serif", backgroundColor: '#05070a' }}>
      ...
      {/* Foto piloto */}
      <div className="absolute inset-x-0 top-[70px] z-[5] flex justify-center" style={{ height: '46%' }}>
        {data.image ? (
      ...
      <div className="absolute left-0 w-full z-20 px-3" style={{ top: '61%' }}>
        <div className="relative flex items-center">
          <div className="w-4 h-8 flex-shrink-0" style={{ backgroundColor: color, transform: 'skewX(-20deg)', marginRight: '-6px' }} />
          <div className="flex-1 py-2 px-3 text-center" style={{ backgroundColor: 'rgba(6,10,14,0.86)', borderTop: `1px solid ${color}55`, borderBottom: `1px solid ${color}55` }}>
            <div className={`text-white font-black italic uppercase tracking-tight leading-none ${longName ? 'text-xl' : 'text-2xl'}`}>{data.nickname || 'PILOTO'}</div>
            <div className="text-[10px] text-white/50 uppercase tracking-[0.25em] mt-1">{data.realName}</div>
          </div>
          <div className="w-4 h-8 flex-shrink-0" style={{ backgroundColor: color, transform: 'skewX(-20deg)', marginLeft: '-6px' }} />
        </div>
        <div className="text-center text-[10px] font-bold uppercase tracking-[0.3em] text-white/70 mt-1.5">{data.league}</div>
      </div>

      {/* Auto protagonista, nítido y grande */}
      <div className="absolute left-1/2 -translate-x-1/2 w-[94%] h-[92px] z-[15] pointer-events-none" style={{ top: '69.5%' }}>
        <img src={getExportUrl(team.car)} alt="" className="w-full h-full object-contain drop-shadow-[0_10px_20px_rgba(0,0,0,0.7)]"
          style={{ transform: `scale(${team.carScale ?? 1})` }}
          referrerPolicy="no-referrer" crossOrigin={getCrossOrigin(getExportUrl(team.car))} />
      </div>

      {/* Barra de estadísticas */}
      <div className="absolute left-3 right-3 z-20 flex items-stretch justify-between rounded-xl px-2 py-2.5" style={{ bottom: '38px', backgroundColor: 'rgba(6,10,14,0.82)', border: `1px solid ${color}44` }}>
      ...
      {/* Footer */}
      <div className="absolute bottom-[10px] left-0 w-full flex items-center justify-center gap-2 z-20">
